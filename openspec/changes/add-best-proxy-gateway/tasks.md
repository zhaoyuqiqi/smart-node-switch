...
- [x] 6.1 实现 `GET /proxy`:返回固定代理地址 + 当前 best 完整信息;无可用节点时明确响应(含单测)
- [x] 6.2 `index.ts` 接入 relay、蓝绿编排、Clash API 客户端的生命周期

## 7. 集成与收尾

- [x] 7.1 端到端验证:起服务 → /nodes 含完整信息 → 占用部分端口仍启动 → Python 经固定代理出网 → 触发刷新代理不中断
- [x] 7.2 更新 README:新增配置项、/proxy 用法、Python 代理示例、Docker 两端口暴露说明

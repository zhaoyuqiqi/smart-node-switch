...
- [ ] **Step 7: 修 monitor.test 的 makeNode**
...
- [ ] **Step 1: 写失败测试 — 四 parser 保存 originalUri**
...
- [ ] **Step 5: 写失败测试 — API 补全 raw/originalUri**
...
- [ ] **Step 1: 写失败测试（用真实 Bun.serve mock HTTP）**
...
- [ ] **Step 1: 写失败测试（本地 echo TCP 上游 A/B）**
...
- [ ] **Step 1: 写失败测试（注入 fake spawn，不起真 sing-box）**
...
- [ ] **Step 1: 写失败测试 — best 变化切 selector**
...
- [ ] **Step 2: 在 monitor.ts 加 setOrchestrator 并放开 opts**
...
4. Python 经固定代理出网：
   ...
   ```
   预期打印 `204`。
5. 刷新期间不中断：开一个长连接（如 `while true; do curl -s -x "$PROXY" http://www.gstatic.com/generate_204 -o /dev/null -w "%{http_code}\n"; sleep 1; done`），手动触发刷新（让可用比例跌破阈值或重启订阅源），观察循环不出现连接被拒/中断窗口。
